...
import React, { memo, useEffect, useRef, useState, FormEvent, Ref, useCallback } from 'react';
...
function ControlTray({trayRef, onToggleTraffic, isTrafficVisible}: ControlTrayProps) {
  ...
  const handleMyLocationClick = useCallback(() => {
    if (userLocation) {
      // Fly to a view similar to an establishing shot, but a bit closer
      setCameraTarget({
        center: { ...userLocation, altitude: 1000 },
        range: 5000,
        tilt: 45,
        heading: 0,
        roll: 0,
      ...
      useLogStore.getState().addTurn({
        role: 'system',
        text: 'Centering map on your current location.',
        isFinal: true,
      });
    } else {
      useLogStore.getState().addTurn({
        role: 'system',
        text: 'Could not center map. Your location is not available.',
        isFinal: true,
      });
    }
  ...
  return (
    <section className="control-tray" ref={trayRef}>
      <nav className={cn('actions-nav', { 'text-entry-visible-landscape': isLandscape && isTextEntryVisible })}>
        ...
        <button
          type="button"
          className={cn('action-button', { 'traffic-on': isTrafficVisible })}
          onClick={onToggleTraffic}
          title={isTrafficVisible ? 'Hide traffic' : 'Show traffic'}
          aria-label={isTrafficVisible ? 'Hide traffic' : 'Show traffic'}
        >
